...
async def auto_submit_account(job, account, chat_id=None, tier="owner"):
    ...
    try:
        ...
        if amazon_cookies_env:
            try:
                cookies = json.loads(amazon_cookies_env)
                log.info(f"✅ Loaded {len(cookies)} cookies from env")
            except Exception as e:
                log.warning(f"⚠️ Env cookie error: {e}")

        if not cookies:
            cookies = load_cookies()

        if not cookies:
            log.warning("❌ No cookies — sending manual alert")
        ...
        if not auth.get("token"):
            log.warning("❌ No HVH_ACCESS_TOKEN — cookies may be expired")
            await tg_alert(job, "ready", chat_id=cid)
            return
